<template>
  ...
  <aside class="fixed lg:static inset-y-0 left-0 z-40 w-80 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col transition-transform duration-300 ease-in-out shadow-sm lg:shadow-none overflow-hidden" :class="uiStore.isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'">
    
    <a href="/" class="h-28 flex items-center px-6 border-b border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors cursor-pointer shrink-0">
      
      <img v-if="workspaceStore.currentWorkspace?.logo_url" :src="workspaceStore.currentWorkspace.logo_url" class="w-14 h-14 object-contain mr-4 drop-shadow-sm flex-shrink-0" />
      
      <div v-else class="w-14 h-14 rounded-2xl bg-blue-600 flex items-center justify-center text-white font-extrabold text-2xl mr-4 shadow-md flex-shrink-0">
        {{ workspaceStore.currentWorkspace?.name.charAt(0).toUpperCase() || 'W' }}
      </div>
      
      <div class="flex-1 overflow-hidden flex items-center">
        <img v-if="workspaceStore.currentWorkspace?.wordmark_url" :src="workspaceStore.currentWorkspace.wordmark_url" class="h-10 object-contain max-w-full" alt="Wordmark" />
        <span v-else class="font-extrabold text-2xl text-gray-900 dark:text-white truncate tracking-tight">
    ...
    <nav class="flex-1 overflow-y-auto p-4 space-y-6">
      <div>
        <router-link to="/favorites" class="flex items-center px-3 py-3 text-lg font-bold rounded-xl transition-colors" :class="$route.path === '/favorites' ? 'bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'">
          <HeartIcon class="mr-4 flex-shrink-0 h-6 w-6" :class="$route.path === '/favorites' ? 'text-red-500 fill-current' : 'text-gray-400'" /> Favorites
        </router-link>
      </div>

      <div>
        <div class="flex items-center justify-between px-3 mb-3">
          <p class="text-sm font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">Channels</p>
          <button v-if="uiStore.isEditMode" @click="openChannelModal()" class="text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/30 p-2 rounded-lg transition-colors" title="Add Channel">
            <PlusIcon class="w-5 h-5" />
          </button>
        </div>
        
        <draggable v-model="workspaceStore.channels" item-key="id" :disabled="!uiStore.isEditMode" @end="workspaceStore.saveChannelOrder" class="space-y-2" handle=".chan-drag-handle">
          <template #item="{ element: channel }">
            <div class="flex flex-col">
              
              <div class="group flex items-center justify-between px-3 py-4 text-xl font-black rounded-2xl text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors cursor-pointer" :class="[uiStore.isEditMode ? 'border border-transparent hover:border-gray-200 dark:hover:border-gray-700 bg-white dark:bg-gray-900 shadow-sm' : '', workspaceStore.activeChannel?.id === channel.id && $route.path === '/' ? 'bg-blue-50/60 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400' : '']">
                ...
                <div @click="() => { workspaceStore.setActiveChannel(channel); router.push('/'); }" class="flex items-center flex-1 overflow-hidden transition-colors">
                  <GripVerticalIcon v-if="uiStore.isEditMode" class="chan-drag-handle w-6 h-6 mr-2 text-gray-400 cursor-grab active:cursor-grabbing hover:text-gray-600 flex-shrink-0" />
                  
                  <img v-if="channel.icon_url" :src="channel.icon_url" class="w-12 h-12 mr-4 object-cover rounded-xl shadow-md border border-gray-200/50 dark:border-gray-700/50 flex-shrink-0" />
                  <div v-else class="w-12 h-12 mr-4 rounded-xl shadow-md border border-gray-200/50 dark:border-gray-700/50 flex items-center justify-center flex-shrink-0 text-xl font-extrabold" :class="workspaceStore.activeChannel?.id === channel.id && $route.path === '/' ? 'bg-blue-600 text-white border-blue-600' : 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 group-hover:bg-gray-300 dark:group-hover:bg-gray-600 transition-colors'">
                ...
                <div v-if="uiStore.isEditMode" class="hidden group-hover:flex items-center space-x-1 ml-2">
                  <button @click.stop="openChannelModal(channel)" class="p-2 text-gray-400 hover:text-blue-500 hover:bg-blue-100 dark:hover:bg-gray-700 rounded-lg transition-colors"><Edit2Icon class="w-5 h-5" /></button>
                  <button @click.stop="workspaceStore.deleteChannel(channel.id)" class="p-2 text-gray-400 hover:text-red-500 hover:bg-red-100 dark:hover:bg-gray-700 rounded-lg transition-colors"><Trash2Icon class="w-5 h-5" /></button>
                </div>
              </div>

              <div v-if="workspaceStore.activeChannel?.id === channel.id && $route.path === '/'" class="ml-12 mt-3 mb-5 space-y-2 relative border-l-2 border-gray-200 dark:border-gray-800 pl-5">
                <draggable v-model="workspaceStore.categories" item-key="id" :disabled="!uiStore.isEditMode" @end="workspaceStore.saveCategoryOrder" class="space-y-1" handle=".cat-drag-handle">
                  <template #item="{ element: category }">
                    <div class="group flex items-center justify-between py-3 px-4 text-lg font-bold rounded-xl cursor-pointer transition-colors" :class="[uiStore.isEditMode ? 'hover:bg-gray-100 dark:hover:bg-gray-800' : '', workspaceStore.activeCategory?.id === category.id ? 'text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-800' : 'text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200']">
                      ...
                      <div v-if="editingCategoryId !== category.id" @click="workspaceStore.setActiveCategory(category)" class="flex items-center flex-1 overflow-hidden">
                        <GripVerticalIcon v-if="uiStore.isEditMode" class="cat-drag-handle w-5 h-5 mr-2 text-gray-400 cursor-grab active:cursor-grabbing hover:text-gray-600 flex-shrink-0" />
                        <span class="truncate">{{ category.name }}</span>
                      </div>

                      <div v-else class="flex items-center flex-1 gap-2">
                        <input v-model="editCategoryName" @keydown.enter.prevent="submitEditCategory" class="w-full bg-white dark:bg-gray-700 border border-blue-500 rounded px-3 py-2 outline-none text-base" autofocus />
                        <button @click.stop="submitEditCategory" class="text-green-500 hover:text-green-600 p-2"><CheckIcon class="w-5 h-5" /></button>
                        <button @click.stop="editingCategoryId = null" class="text-gray-400 hover:text-gray-600 p-2"><XIcon class="w-5 h-5" /></button>
                      </div>

                      <div v-if="uiStore.isEditMode && editingCategoryId !== category.id" class="hidden group-hover:flex items-center space-x-1 ml-2">
                        <button @click.stop="startEditCategory(category)" class="p-2 text-gray-400 hover:text-blue-500 rounded"><Edit2Icon class="w-4 h-4" /></button>
                        <button @click.stop="workspaceStore.deleteCategory(category.id)" class="p-2 text-gray-400 hover:text-red-500 rounded"><Trash2Icon class="w-4 h-4" /></button>
                      </div>
                    </div>
                  </template>
                </draggable>

                <div v-if="uiStore.isEditMode" class="mt-2">
                  <div v-if="isAddingCategory" class="flex items-center gap-2 mt-2 px-2">
                    <input v-model="newCategoryName" @keydown.enter.prevent="submitAddCategory" placeholder="Category..." class="w-full bg-white dark:bg-gray-800 border border-blue-500 rounded-lg px-3 py-2 outline-none text-base" autofocus />
                    <button @click="submitAddCategory" class="p-2 text-green-500 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg"><CheckIcon class="w-5 h-5" /></button>
                    <button @click="isAddingCategory = false; newCategoryName = ''" class="p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg"><XIcon class="w-5 h-5" /></button>
                  </div>
                  <button v-else @click="isAddingCategory = true" class="flex items-center px-4 py-3 mt-2 text-base font-bold rounded-xl text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors w-full">
                    <PlusIcon class="mr-3 flex-shrink-0 h-5 w-5" /> Add Category
                  </button>
                </div>
              </div>
        ...
        <button v-if="uiStore.isEditMode" @click="openChannelModal()" class="w-full flex items-center px-4 py-4 mt-4 text-base font-bold rounded-2xl text-blue-600 dark:text-blue-400 border-2 border-dashed border-blue-200 dark:border-blue-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
          <PlusIcon class="mr-3 flex-shrink-0 h-5 w-5" /> Add Channel
        </button>
      </div>
    </nav>
        ...
        <div>
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex-1">
              ...
              <button type="button" @click="$refs.iconInput.click()" class="text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-xl font-bold hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors shadow-sm">Choose Image</button>
            </div>
          </div>
        </div>
